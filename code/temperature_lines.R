...
t_data %>% ggplot(aes(
  ...
  labs(
    x = NULL,
    y = " Temperature change since pre-inductrial time [\u00B0C]",
    title = glue("Global temperature change since {min(t_data$year)} by month")
  ) +
  theme(
...
ggsave("figures/temperature_lines.png",
       width = 8,
       height = 4.5)
